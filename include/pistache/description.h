...
#define DATA_TYPE                                                       \
      TYPE(Integer, std::int32_t             , "integer", "int32")      \
        ...
        COMPLEX_TYPE(FormData, "file",  "formData")
...
#define PARAMETER_LOCATIONS                     \
    PARAMETER_LOCATION(query)                   \
      ...
      PARAMETER_LOCATION(formData)              \
      PARAMETER_LOCATION(body)                  \
    
#define PARAMETER_LOCATION(e) e,
...
#undef COMPLEX_TYPE
      ...
      struct Parameter {
        Parameter(
                  std::string name, std::string description, ParameterLocation in
      ...
      struct Response {
        ...
        Response(Http::Code statusCode, std::string description,std::shared_ptr<SchemaObj> sch);
      ...
      struct PathBuilder {
        ...
        template<typename T>
        PathBuilder& parameter(std::string name, std::string description, ParameterLocation in) {
          path_->parameters.push_back(Parameter::create<T>(std::move(name), std::move(description),in));
          return *this;
        }
