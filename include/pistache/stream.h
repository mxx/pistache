...
namespace Pistache {
  ...
  DynamicStreamBuf(): maxSize_(std::numeric_limits<uint32_t>::max())
      {
        reserve(Const::MaxBuffer);
        data_.clear();
        Base::setg(data_.data(), data_.data(), data_.data()+data_.capacity());
      }
  DynamicStreamBuf(
                   size_t size,
  ...
  DynamicStreamBuf(DynamicStreamBuf&& other)
    ...
    void clear() {
      data_.clear();
      this->setp(data_.data(), data_.data() + data_.capacity());
    }
    
    virtual std::streamsize xsputn (const char_type* s, std::streamsize n) {
      if (feed(s,n))
        {
          setp(data_.data()+data_.size(),data_.data() + data_.capacity());
          return n;
        }
        return 0;
    }
    bool feed(const char* data, size_t len) {
      //if buffer empty, direct copy
      /* if (this->gptr() && (this->gptr() == this->egptr())) { */
      /*   memcpy(data_.data(),data,len); */
      /*   Base::setg(data_.data(), data_.data(), data_.data() + len); */
      /*   return true; */
      /* } */
      auto it = data_.begin();
