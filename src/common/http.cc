...
#undef METHOD
      ...
      BodyStep::apply(StreamCursor& cursor) {
        ...
        string str = MIME(Multipart, FormData).toString();
        if (ct && ((ct->mime().toString().find(MIME(Multipart, FormData).toString())!=string::npos)))
      ...
      State
      BodyStep::parseMultipartFormHeads(StreamCursor& cursor,
                                        std::shared_ptr<Header::ContentDisposition>& h)
      {
        //StreamCursor::Revert revert(cursor);
        ...
        while (!cursor.eol()) {
          ...
          if (Header::Registry::isRegistered(name)) {
            std::shared_ptr<Header::Header> header = Header::Registry::makeHeader(name);
            header->parseRaw(cursor.offset(start), cursor.diff(start));
            if (strcmp(header->name(),"Content-Disposition")==0)
              h = std::static_pointer_cast<Header::ContentDisposition>(header);
          }
          if (h->type() != "form-data" && h->params().empty())
            raise("Malformed request body in multipart form data", Code::Bad_Request);              
          // CRLF
          if (!cursor.advance(2)) return State::Again;
          //headerRevert.ignore();
        };
